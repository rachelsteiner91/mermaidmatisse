...
class Artworks(Resource):
    def get(self):
        ...
        artworks_dict = [a.to_dict(only=(
            'id', 'title', 'medium',  'artists.name', 'style.style_type', 'image')) for a in artworks]
        res = make_response(
            artworks_dict,
            200
        )
...
class Artists(Resource):
    def get(self):
        ...
        artists_dict = [a.to_dict(only=('id', 'name', 'medium', 'image',
                                  'description', 'artworks.title', 'artworks.image')) for a in artists]
        res = make_response(
            artists_dict,
            200
        )
...
class OneArtist(Resource):
    def get(self, id):
        ...
        res = make_response(
            artist.to_dict(only=('id', 'name', 'medium', 'image',
                           'description', 'artworks.title', 'artworks.image')),
            200
        )
        return res
...
class NewCollection(Resource):
    def get(self):
        ...
        collections_dict = [
            c.to_dict(only=("id", "artwork_id", "user_id",)) for c in collections]
...
class Users(Resource):
    def get(self):
        ...
        users_dict = [
            u.to_dict(only=('id', 'name', 'username', 'role')) for u in users]
        res = make_response(
            users_dict,
            200
        )
